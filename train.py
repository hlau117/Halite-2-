...
import numpy as np
import pandas as pd
from cnnbot.parsing import parse
from cnnbot.map_transform import map_transformer
...
def fetch_data_dir(directory, limit):
    ...
    replay_files = sorted([f for f in os.listdir(directory) if
                           os.path.isfile(os.path.join(directory, f)) and f.startswith("halite-")])
...
def main():
    ...
    nn = ConvNeuralNet(cached_model=args.cache, seed=args.seed)
    ...
    if args.bot_to_imitate is None:
        ...
        args.bot_to_imitate = max(players_games_count, key=players_games_count.get)
    print("Bot to imitate: {}.".format(args.bot_to_imitate))
    data_input= []
