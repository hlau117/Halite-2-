...
def format_data_for_training(data):
    ...
    print('input shape',np.array(training_input).shape)
    print('output shape', np.array(training_output).shape)
...
def parse(all_games_json_data, bot_to_imitate=None, dump_features_location=None):
    ...
    for json_data in all_games_json_data:
        ...
        game_training_data = []
        cnn_input = map_transformer(json_data, bot_to_imitate)
        ...
        for idx in range(len(frames) - 1):
            ...
            game_training_data.append((cnn_input[idx],allocations))
